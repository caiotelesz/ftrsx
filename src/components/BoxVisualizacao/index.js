...
export default function BoxVisualizacao({ item, relatedItems }) {
  const imgSrc = item.imagem
    ? `${API_ADDRESS}/${item.imagem.replace(/\\/g, "/")}`
    : "assets/images/pecas/teeblack.png";
    ...
    <section className="section-visu">
      <div className="card-visu">
        ...
        <div className="box2">
          ...
          <p>{item.descricao ?? "descrição"}</p>

          <div className="botaoTamanho">
            <h2>Tamanho: </h2>
            <button
              className={tamanhoSelecionado === "P" ? "selecionado" : ""}
            ...
            <button
              className={tamanhoSelecionado === "M" ? "selecionado" : ""}
            ...
            <button
              className={tamanhoSelecionado === "G" ? "selecionado" : ""}
            ...
            <button
              className={tamanhoSelecionado === "GG" ? "selecionado" : ""}
        ...
        <div className="related-items">
          ...
          <div className="related-items-list">
            {relatedItems.map((relatedItem) => (
              <div key={relatedItem.id} className="card">
                <div className="content">
                  <Link to={`/visualizacao/${relatedItem.id}`}>
                    <img
                      src={`${API_ADDRESS}/${relatedItem.imagem.replace(
                        /\\/g,
                        "/"
                      )}`}
                      alt={relatedItem.nome}
                    />
                    <div>
                      <h1 className={relatedItem.nome}>
                        {relatedItem.nome ?? "Nome do item"}
                      </h1>
                      <span>
                        BRL{" "}
                        {relatedItem.preco?.toFixed(2) ??
